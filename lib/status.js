...
StatusController.prototype.show = function(req, res) {
  ...
  case 'getNetworkHashrate':
    this.getNetworkHashrate(function(err, result) {
      if (err) {
...
StatusController.prototype.getNetworkHashrate = function(callback) {
  this.node.services.bitcoind.getInfo(function(err, info) {
    if (err) {
      return callback(err);
    }
    const diff = info.difficulty;
    const pre_nh = diff * Math.pow(2,32) / 60;
    const nethash = pre_nh * 2 / 1000000;
    callback(null, {
      nethash: nethash
    });
  });
...
//     callback(null, {
//       supply: (info.blocks * 8) + 2000000 //full premine added to totalsupply
//     });
    callback(null, 
      //adjusted for vita update and premine added to totalsupply
      (440000 * 8) + 2000000 + ((info.blocks - 440000) * 10)
    );
  });
};
